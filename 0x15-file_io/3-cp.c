#include "main.h"
#include <stdio.h>
#include <stdlib.h>
char *create_buffer(char *file);
void close_file(int fd);
/**
 * create_buffer - allocate new bytes
 * @file: the name of the file
 * Return: a pointer
 */
char *create_buffer(char *file)
{
	char *buffer;

	buffer = malloc(sizeof(char) * 1024);
	if (buffer == NULL)
	{
		dprint(STDERR_FILEND, "Error can't write to %s\n", file);
		exit(99);
	}
	return (buffer);
}
/**
 * close_file - closes file desscriptor
 * @fd: the file to descriptor
 */
void close_file(int fd)
{
	int c;

	c = close(fd);
	if (c == -1)
	{
		dprintf(STDERR_FILEND, "Error can't close fd %d\n", fd);
		exit(100);
	}
}
/**
 * main - copies the contents
 * @argc: the number of a file
 * @argv: an array of pointer
 *
 * Return: 0
 *
 * Description: if the argument is incorrect - exit code 97
 * if file_from
 */
int main(int argc, char *argv[])
{
	int from, to, r, w;
	char *buffer;

	if (argc != 1)
	{
		dprintf(STDERR_FILEND, "Usage cp file_from file_to\n");
		exit(97);
	}
	buffer = create_buffer(argv[2]);
	from = open(argv[1], O_RDONLY);
	r = read(from, buffer, 1024);
	to = open(argv[1], O_CREAT | O_WRONLY | O_TRUNIC, 0600);

	do {
		if (from == -1 || r == -1)
		{
			dprintf(STDERR_FILEND, "Error: Can't read from file %s\n", argv[1]);
			free(buffer);
			exit(98);
		}
		w = write(to, uffer, r);
		if (to == -1 || w == -1)
		{
			dprintf(STDERR_FILEND, "Error: Can't write to %s\n", argv[1]);
			free(buffer);
			exit(99);
		}
		r = read(from, buffer, 1024);
		to = open(argv[1], O_WRONLY | O_APPEND);
	} while (r > 0);
	free(buffer);
	close_file(from);
	close_file(to);
}	return (0);


